...
\begin{document}
	...
	\begin{itemize}
		...
		\begin{itemize}
			...
			\item Detection-by-tracking: (recursive tracking, filtering, model-based tracking, ...)
			\begin{itemize}
				\item Tracking with dynamics
				\item image measurement of estimated object position + position prediction by dynamics (expectation of object's motion pattern)
				\item Dynamics model: predict where object will occure
				\item Intent: restrict search (do less work), improved estimates (measurement noise tempered by smooth dynamics priors)
				\item Assumptions: continuous motion patterns
				\newline \todo{add assumptions (Folie 14)}
			\end{itemize}
			
			\item Tracking as inference
			\begin{itemize}
				\item Vollständiger Zustand ist nicht bekannt
				\newline hidden state X: true parameters we care about
				\item measurement Y: noisy observation, results from underlying state (X)
				\item state vs. observation
				\item goal: recover most likely state $X_t$ given
				\newline all observations seen so far
				\newline knowledge about dynamics of state transitions
				\item Independence assumption: Markov property (only last state relevant)
				\newline measurement at time t only dependens on current state in time t
				\item \textcolor{red}{periodische Ereignisse sind problematisch mit Markov assumption, warum dennoch? $\rightarrow$ Rechenzeit einsparen (längere Historie dauert länger) + funktioniert}
				\item Prediction: $P(X_t | y_0, ..., y_{t-1})$ + Correction: $P(X_t | y_0, ..., y_t)$ 
			\end{itemize}
			
		\end{itemize}
		
		\item Linear dynamic model
		\begin{itemize}
			\item a priori knowledge about system dynamics model (evolution of state over time) + measurement model (at every time step: noisy measurement of state)
		\end{itemize}
